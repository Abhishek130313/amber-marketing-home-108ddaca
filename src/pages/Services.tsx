...
import { organizationSchema, breadcrumbSchema, serviceSchema } from "@/lib/schemas";
  ...
  return (
    <button onClick={() => setOpen(!open)} className="w-full text-left bg-card border border-border rounded-2xl p-5 hover:shadow-soft transition">
      <div className="flex items-center justify-between gap-4">
        ...
        <ChevronDown className={`w-5 h-5 text-muted-foreground transition-transform ${open ? "rotate-180" : ""}`} aria-hidden="true" />
      </div>
...
const servicesJsonLd = [
  ...
  serviceSchema("Website Development India", "Professional business website design and development for small businesses in India starting from ₹7,999.", "/services"),
  serviceSchema("SEO Services India", "On-page, off-page and technical SEO to rank your business on the first page of Google in India.", "/services"),
  serviceSchema("Digital Marketing India", "Google Ads, Facebook Ads, Instagram marketing and lead generation for Indian small businesses.", "/services"),
  {
    ...
    mainEntity: [
      {
        ...
        acceptedAnswer: { "@type": "Answer", text: "Websbond builds professional business websites starting from ₹7,999. E-commerce websites start from ₹14,999. All prices are transparent with no hidden charges." },
      },
      {
        "@type": "Question",
        name: "How long does it take to build a website?",
        acceptedAnswer: { "@type": "Answer", text: "A standard business website takes 5-7 working days. E-commerce websites take 10-15 working days. Rush delivery is available." },
      },
      {
        "@type": "Question",
        name: "Do you provide SEO services for small businesses?",
        acceptedAnswer: { "@type": "Answer", text: "Yes, Websbond provides complete SEO services including on-page SEO, off-page link building, technical SEO, and local SEO for small businesses across India." },
      },
      {
        "@type": "Question",
        name: "Does Websbond provide support after website delivery?",
        acceptedAnswer: { "@type": "Answer", text: "Yes, we provide 24/7 human support via WhatsApp, call and email. We also offer monthly website maintenance plans." },
      },
    ],
  },
];
...
const ServicesPage = () => (
  <Layout>
    <SEO
      title="Website Development, SEO & Digital Marketing Services | Websbond India"
      description="Affordable website design from ₹7,999, Google SEO, Facebook & Google Ads, brand identity and social media management for small businesses in India. No hidden charges. Free consultation."
      path="/services"
      ...
      <div className="container grid lg:grid-cols-2 gap-12 py-14 lg:py-20 items-center">
        <div>
          <div className="inline-flex items-center gap-2 bg-accent/15 text-foreground font-semibold text-xs uppercase tracking-wider px-4 py-2 rounded-full">
            <Briefcase className="w-3.5 h-3.5 text-accent" aria-hidden="true" /> Services
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            ...
            <Link to="/our-work" className="inline-flex items-center gap-2 bg-card border border-border font-semibold px-7 py-4 rounded-2xl hover:shadow-soft transition">
              <span className="grid place-items-center w-7 h-7 rounded-full bg-accent text-accent-foreground" aria-hidden="true"><Play className="w-3 h-3 fill-current" /></span>
              Hamara Kaam Dekhein
            </Link>
          </div>
          <div className="mt-8 flex flex-wrap gap-x-6 gap-y-2 text-sm font-medium">
            {["No Contract","No Hidden Charges","Cancel Anytime"].map((t) => (
              <div key={t} className="flex items-center gap-2"><Check className="w-4 h-4 text-green-500" aria-hidden="true" /> {t}</div>
            ))}
          </div>
        </div>
        <div className="relative h-[420px] lg:h-[500px]">
          ...
          {[
            ...
            <div key={val} className={`absolute ${cls} bg-card rounded-2xl shadow-card p-4 flex items-start gap-3 w-52 z-20`}>
              <div className="w-9 h-9 rounded-xl bg-accent/15 text-brand grid place-items-center shrink-0" aria-hidden="true"><Icon className="w-4 h-4" /></div>
              <div>
        ...
        {services.map(({ icon: Icon, title, desc, features, color }) => (
          <article key={title} className="bg-card rounded-3xl p-7 border border-border shadow-soft hover:shadow-card hover:-translate-y-1 transition-all">
            <div className="flex items-center gap-3 mb-4">
              <div className={`w-12 h-12 rounded-xl grid place-items-center ${color}`} aria-hidden="true"><Icon className="w-6 h-6" /></div>
              <h3 className="font-display font-bold text-lg">{title}</h3>
            </div>
            <p className="text-sm text-muted-foreground mb-5">{desc}</p>
            <ul className="space-y-2 mb-6">
              {features.map((f) => (
                <li key={f} className="flex items-center gap-2 text-sm"><Check className="w-4 h-4 text-green-500" aria-hidden="true" /> {f}</li>
              ))}
            </ul>
    ...
    <section className="container py-16">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {pricing.map(({ name, icon: Icon, color, desc, price, features, highlight }) => (
          <div key={name} className={`relative bg-card rounded-3xl p-7 border-2 ${highlight ? "border-accent shadow-float" : "border-border shadow-soft"}`}>
            ...
            <div className={`w-12 h-12 rounded-xl grid place-items-center mb-4 ${color}`} aria-hidden="true"><Icon className="w-6 h-6" /></div>
      ...
      <div className="mt-8 flex flex-wrap justify-center gap-x-8 gap-y-2 text-sm">
        {["No Contract","No Hidden Charges","Cancel Anytime"].map((t) => (
          <div key={t} className="flex items-center gap-2 text-muted-foreground"><Check className="w-4 h-4 text-green-500" aria-hidden="true" /> {t}</div>
        ))}
      </div>
    </section>
